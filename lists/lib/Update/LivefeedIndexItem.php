...
final class LivefeedIndexItem extends Stepper
{
	...
	public function execute(array &$result)
	{
		if (!(
			...
			&& Option::get('lists', 'needLivefeedIndexItem', 'Y') == 'Y'
		))
		{
			return false;
		}
		...
		$params = Option::get("lists", "livefeedindexitem", "");
		...
		if ($params["count"] > 0)
		{
			$result["title"] = Loc::getMessage("FUPD_LF_LISTS_ITEM_EVENT_INDEX_TITLE");
			$result["progress"] = 1;
			$result["steps"] = "";
			...
			if ($found)
			{
				Option::set("lists", "livefeedindexitem", serialize($params));
				$return = true;
			}
			...
			if ($found === false)
			{
				Option::delete("lists", array("name" => "livefeedindexitem"));
				Option::set('lists', 'needLivefeedIndexItem', 'N');
			}
		}
		return $return;
	}
