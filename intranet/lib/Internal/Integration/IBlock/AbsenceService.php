...
class AbsenceService
{
	...
	public function get(int $userId, int $iblockId, Date $day): ?Absence
	{
		...
		if ($absence = $res->Fetch())
		{
			$xmlId = $absenceTypeEnumXmlId[$absence['PROPERTY_ABSENCE_TYPE_ENUM_ID']] ?? '';

			return new Absence(
				$userId,
				new Date($absence['ACTIVE_FROM']),
				new Date($absence['ACTIVE_TO']),
				$absence['NAME'],
				$xmlId,
	...
	{
		return Loc::getMessage('INTRANET_ABSENCE_SERVICE_TYPE_' . $type);
	}
