...
class InvitationRepository
{
	 ...
	 */
	public function getUserById(int $userId): ?Invitation
	{
		//TODO: need to add a cache
		$model = InvitationTable::query()->where('USER_ID', $userId)->fetchObject();
		if (!$model)
		{
			return null;
		}
	...
	public function makeEntityByModel($model): Invitation
	{
		return new Invitation(
			...
			$model->getIsMass(),
			$model->getIsDepartment(),
			$model->getIsIntegrator(),
			$model->getIsRegister(),
			id: $model->getId(),
