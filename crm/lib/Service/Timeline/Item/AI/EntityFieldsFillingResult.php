...
final class EntityFieldsFillingResult extends Base
{
	...
	public function getButtons(): ?array
	{
		...
		if ($this->getActivityProvider() === Call::getId())
		{
	...
	protected function getOpenAction(): ?Action
	{
		...
		if ($jobResult?->isSuccess() === true)
		{
			return (new JsEvent('EntityFieldsFillingResult:OpenAiFormFill'))
				...
				->addActionParamString('summarizeJobId', $jobResult?->getParentJobId())
			;
		}
	...
	protected function getJobResult(): ?Result
	{
		...
		if ($this->getActivityProvider() === Call::getId())
		{
			return $jobRepository->getFillItemFieldsFromCallTranscriptionResult(
		...
		return $jobRepository->getFillItemFieldsFromCallTranscriptionResultById($jobId);
	}
}
