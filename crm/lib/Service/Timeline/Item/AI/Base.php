...
{
	public const SUPPORTED_ACTIVITY_PROVIDER = SummarizeCallTranscription::SUPPORTED_ACTIVITY_PROVIDER_IDS;

	private const HELPDESK_ABOUT_JOB_LANG = '20423978';

	abstract protected function getAITypeId(): string;
	...
	{
		return sprintf('AI:%s', $this->getAITypeId());
	}
	...
	final protected function getJobId(): ?int
	{
		return $this->getModel()->getSettings()['JOB_ID'] ?? null;
	}

	final protected function getActivityProvider(): ?string
	{
		$providerId = $this->getAssociatedEntityModel()?->get('PROVIDER_ID');
		if (in_array($providerId, self::SUPPORTED_ACTIVITY_PROVIDER, true))
		{
			return $providerId;
		}
	...
	private function buildBaseActivityBlock(): ?ContentBlock
	{
		...
		{
			$userCode = $this->getModel()->getAssociatedEntityModel()?->get('PROVIDER_PARAMS')['USER_CODE'];
			$subject = OpenLine::getChatName($userCode);
		}
