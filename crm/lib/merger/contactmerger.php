...
class ContactMerger extends EntityMerger
{
	...
	protected function rebind($seedID, $targID)
	{
		...
		Timeline\AI\Entry::rebind(\CCrmOwnerType::Contact, $seedID, $targID);
	...
	protected function prepareCollisionMessageFields(array &$collisions, array &$seed, array &$targ): array
	{
		...
		): ?string
		{
			...
			))
			{
				$messages[] = Loc::getMessage(
					'CRM_CONTACT_MERGER_COLLISION_READ_UPDATE_PERMISSION',
					$replacements,
					$languageId,
				);
			}
			...
			{
				$messages[] = Loc::getMessage(
					'CRM_CONTACT_MERGER_COLLISION_READ_PERMISSION',
					$replacements,
					$languageId,
				);
			}
			...
			{
				$messages[] = Loc::getMessage(
					'CRM_CONTACT_MERGER_COLLISION_UPDATE_PERMISSION',
					$replacements,
					$languageId,
				);
			}
		...
		$notifyMessageSubjectCallback = function (?string $languageId = null) use (
			$collisions,
			$seed,
			$targ,
		): ?string
		{
			...
			{
				$messages[] = Loc::getMessage(
					'CRM_LEAD_MERGER_COLLISION_SUBJECT',
					$replacements,
					$languageId,
				);
			}
		...
		$notifyMessagePlainTextCallback = function (?string $languageId = null) use (
			$collisions
		): ?string
		{
			self::includeLangFile();
			...
			{
				$messages[] = Loc::getMessage(
					'CRM_LEAD_MERGER_COLLISION_READ_UPDATE_PERMISSION_PLAIN_TEXT',
					null,
					$languageId,
				);
			}
			...
			{
				$messages[] = Loc::getMessage(
					'CRM_LEAD_MERGER_COLLISION_READ_PERMISSION_PLAIN_TEXT',
					null,
					$languageId,
				);
			}
			...
			{
				$messages[] = Loc::getMessage(
					'CRM_LEAD_MERGER_COLLISION_UPDATE_PERMISSION_PLAIN_TEXT',
					null,
					$languageId,
				);
			}
