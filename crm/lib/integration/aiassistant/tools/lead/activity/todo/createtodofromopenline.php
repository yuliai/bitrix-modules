...
final class CreateToDoFromOpenLine extends BaseCrmTool
{
	...
	{
		return 'create_lead_todo_from_openline';
	}
	...
	public function getDescription(): string
	{
		return <<<HEREDOC
			...
			The to-do is assigned to the current responsible manager of the lead
		HEREDOC;
	}
	...
	private static function fail(ErrorCollection|Error $errorCollection): string
	{
		...
		return 'Failed to create to-do from open lines for lead: ' . implode(', ', $errorMessages);
	}

	private static function success(ToDo $todo): string
	{
		return "To-do '{$todo->getSubject()}' (ID: {$todo->getId()}) has been successfully created and attached from open lines to the lead";
	}
}
