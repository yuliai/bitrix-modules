...
class DiskManager
{
	 ...
	 * @return string
	 */
	public static function getFileName($fileID): string
	{
		if (!Loader::includeModule('disk'))
		{
			return "[{$fileID}]";
	...
	public static function getFileInfo($fileID, $checkPermissions = true, $options = null): ?array
	{
		if (!Loader::includeModule('disk'))
		{
			return null;
		...
		$ownerID = $options['OWNER_ID'] ?? 0;
		$ownerTypeID = $options['OWNER_TYPE_ID'] ?? \CCrmOwnerType::Undefined;
		...
		if ($ownerID > 0 && \CCrmOwnerType::isDefined($ownerTypeID))
		{
			...
			$viewUri = new Uri('/bitrix/tools/crm_show_file.php');
			$viewUri->addParams($viewUrlParams);
		...
		{
			$securityContext = $file->getStorage()?->getSecurityContext(\CCrmSecurityHelper::getCurrentUserID());
			$canRead =
		...
		return [
			...
			'BYTES' => $file->getSize(),
			'CAN_READ' => $canRead,
			'VIEW_URL' => isset($viewUri) ? $viewUri->getUri() : '',
			'PREVIEW_URL' => isset($previewUrl) ? $previewUrl->getUri() : '',
		];
	}
	...
	protected static function loadFolderModel($storage, $parent, $typeID, $xmlID, $name)
	{
		...
		if (!$folderModel)
		{
			...
			{
				$specificRights =
					Driver::getInstance()->getRightsManager()->getSpecificRights($storage->getRootObject());
	...
	public static function writeFileToResponse($fileID, $options = array())
	{
		...
		\CFile::viewByUser($fileData,
			['force_download' => false, 'cache_time' => 0, 'attachment_name' => $file->getName()]);
	}
