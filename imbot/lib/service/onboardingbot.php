...
class Onboardingbot
{
	...
	private static function sendAnswer($messageFields)
	{
		...
		if (!empty($messageFields['KEYBOARD']))
		{
			if (is_string($messageFields['KEYBOARD']))
			{
				...
				{
					\Bitrix\ImBot\Log::writeToFile('onboardingbot_errors.log', ['error' => 'Invalid JSON in KEYBOARD: ' . $e->getMessage()], 'ONBOARDINGBOT_KEYBOARD_ERROR');
					return 0;
				}
			...
			$keyboard['BOT_ID'] = \Bitrix\ImBot\Bot\Onboardingbot::getBotId();
		...
		$messageId = \Bitrix\Im\Bot::addMessage(['BOT_ID' => ImBot\Bot\Onboardingbot::getBotId()], [
	...
	{
		if (ImBot\Bot\Onboardingbot::isEnabled())
		{
			return true;
		}

		return (bool)ImBot\Bot\Onboardingbot::register();
	}

	public static function updateBotProperties(array $botProperties = []): bool
	{
		if (
			!($botId = \Bitrix\ImBot\Bot\Onboardingbot::getBotId())
			|| empty($botProperties)
		)
		{
			return false;
