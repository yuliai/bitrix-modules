...
{
	private const MAX_RRULE_COUNT = 10000;
	...
	private static
		...
		/** @var $openEventSection Section|null|false */
	...
	public static function Edit($params = [])
	{
		...
		if (!$isNewEvent)
		{
			...
			if (
				isset($entryFields['MEETING'], $currentEvent['MEETING']['CHAT_ID'])
		...
		if (self::CheckFields($entryFields, $currentEvent, $userId))
		{
			...
			{
				self::clearCalendarUserCache($entryFields['OWNER_ID']);
			}
			...
			if ($userId && $params && ($params['overSaving'] ?? null) !== true)
			{
				ServiceLocator::getInstance()
					->get(RequestLogger::class)
					->debug(
						'Edit event',
						[
							'params' => $params,
				...
				if (isset($entryFields['DATE_TO_TS_UTC']) && $entryFields['DATE_TO_TS_UTC'] > $nowUtc)
				{
					...
					)
					{
						...
						)
						{
							...
							CCalendarNotify::Send([
								...
								"userId" => $userId ?? $entryFields['MEETING_HOST'],
								"fields" => $entryFields,
					...
					)
					{
						...
						CCalendarNotify::Send(array(
							...
							"guestId" => $entryFields['OWNER_ID'] ?? null,
							"eventId" => $entryFields['PARENT_ID'] ?? null,
							"userId" => $entryFields['MEETING_HOST'] ?? $userId,
							"isSharing" => ($entryFields['EVENT_TYPE'] ?? null) === Dictionary::EVENT_TYPE['shared'],
							"fields" => $entryFields,
						));
					}
			...
			)
			{
				...
				$parentEventId = $entryFields['PARENT_ID'] ?? $result;
				...
				$isDaylightSavingTimezone = !empty($entryFields['TZ_FROM'])
					? CCalendar::isDaylightSavingTimezone($entryFields['TZ_FROM'])
					: ''
				;
				...
				Util::addPullEvent(
					...
					[
						'fields' => array_merge($entryFields, [
							...
							'IS_DAYLIGHT_SAVING_TZ' => $isDaylightSavingTimezone,
							'COLLAB_ID' => $collabId,
	...
	public static function GetList($params = [])
	{
		...
		if (!$bCache || !isset($resultEntryList))
		{
			...
			$listResult = self::getListOrm($params);
			[$eventList, $parentMeetingIdList, $involvedUsersIdList, $openEventParentIdList] = $listResult;

			if (!empty($params['fetchAttendees']) && !empty($parentMeetingIdList))
			{
				$attendeeListData = self::getAttendeeList($parentMeetingIdList, $openEventParentIdList);
			...
			foreach ($eventList as $event)
			{
				...
				)
				{
					if ($isOpenEvent)
					{
						...
						// cause there is no logic for decline user of open event
			...
			if ($bCache)
			{
				...
				{
					// A temporary fix for http://jabber.bx/view.php?id=215935
					$logger = new \Bitrix\Main\Diag\EventLogger('calendar', 'REMIND_DEBUG');

					$logger->debug('Found Closure. Message: ' . $e->getMessage());

					try
					{
						$logger->debug('Found Closure. Params: ' . \Bitrix\Main\Web\Json::encode($originalParams));
						$logger->debug(
							'Found Closure. To cache: ' . \Bitrix\Main\Web\Json::encode([
								'resultEntryList' => $resultEntryList,
								'userIndex' => $userIndex,
								'dateTimeFormat' => FORMAT_DATETIME,
							])
	...
	private static function getListOrm($params = [])
	{
		...
		$hasOriginalRecursion = $query->getEntity()->hasField('ORIGINAL_RECURSION');
		...
		while ($eventObject = $queryResult->fetchObject())
		{
			[$event, $parentMeetings, $involvedUsers] = self::prepareEventObject(
				...
				$eventObject,
				$hasAttendees,
				$hasOriginalRecursion
			);

			$parentMeetingIdList = [...$parentMeetingIdList, ...$parentMeetings];
			$involvedUsersIdList = [...$involvedUsersIdList, ...$involvedUsers];
		...
		while ($eventObject = $attendeesQueryResult->fetchObject())
		{
			...
			$parentMeetingIdList = [...$parentMeetingIdList, ...$parentMeetings];
			$involvedUsersIdList = [...$involvedUsersIdList, ...$involvedUsers];
			$openEventParentMeetingIdList = [...$openEventParentMeetingIdList, ...$parentMeetings];
		...
		return [
			$eventList,
			array_unique($parentMeetingIdList),
	...
	private static function prepareEventObject(
		int $userId,
		Internals\EO_Event $eventObject,
		bool $hasAttendees,
		bool $hasOriginalRecursion = false
	): array
	{
		...
		$event['IS_DAYLIGHT_SAVING_TZ'] = !empty($event['TZ_FROM'])
			? CCalendar::isDaylightSavingTimezone($event['TZ_FROM'])
			: ''
		;
...
//			$event['~TYPE'] = 'open_event';
		...
		/** @var Internals\EO_EventOriginalRecursion $originalRecursion */
		if (
			$hasOriginalRecursion
			&& $originalRecursion = $eventObject->get('ORIGINAL_RECURSION')
		)
		{
			$event['ORIGINAL_RECURSION_ID'] = $originalRecursion->getOriginalRecursionEventId();
		}
	 ...
	 */
	public static function getAttendeeList($entryIdList = [], array $openEventIdList = []): array
	{
		$attendeeList = [];
		$userIdList = [];
		...
		$entryIdList = is_array($entryIdList)
			? array_map('intval', array_unique($entryIdList))
			: [(int)$entryIdList]
		;

		if (empty($entryIdList) && empty($openEventIdList))
		{
			return [
				'attendeeList' => $attendeeList,
				'userIdList' => $userIdList,
		...
		{
			$queries[] = Internals\EventTable::query()
				->setSelect([
					'USER_ID' => 'OWNER_ID',
					'ID',
					'PARENT_ID',
					'EVENT_MEETING_STATUS' => 'MEETING_STATUS',
					'MEETING_HOST',
				])
				->where('ACTIVE', 'Y')
				->where('CAL_TYPE', 'user')
				->where('DELETED', 'N')
				->whereIn('PARENT_ID', $entryIdList)
				->exec()
			;
		}

		if (!empty($openEventIdList))
		{
			...
			$queries[] = Internals\EventTable::query()
				...
				->setSelect([
					'USER_ID' => 'EVENT_ATTENDEE.OWNER_ID',
					'ID',
					'PARENT_ID',
					'EVENT_MEETING_STATUS' => 'EVENT_ATTENDEE.MEETING_STATUS',
					'MEETING_HOST',
					'ATTENDEE_ID' => 'EVENT_ATTENDEE.ID',
				])
				->where('ACTIVE', 'Y')
				->where('DELETED', 'N')
				->whereIn('PARENT_ID', $openEventIdList)
				->where(
					(new ConditionTree())
						->logic(ConditionTree::LOGIC_OR)
						->where('EVENT_ATTENDEE.MEETING_STATUS', 'Y')
						->whereNot('CAL_TYPE', Dictionary::CALENDAR_TYPE['open_event'])
				)
				->exec()
			;
		}

		// generator for union-like joining queries
		$queryCombineGenerator = static function (array $queryResults): \Generator
		{
		...
		$combinedQuery = $queryCombineGenerator($queries);
		...
		while($entry = $combinedQuery->current())
		{
			...
			$entry["STATUS"] = trim($entry['EVENT_MEETING_STATUS']);
			if (
	...
	public static function getUsersDetails($userIdList = [], $params = [])
	{
		...
		if (!empty($userIdList))
		{
			$userIdList = array_unique(array_map(static fn($userId) => (int)$userId, $userIdList));

			$userList = CCalendar::GetUserList($userIdList);
		}

		foreach ($userList as $userData)
		{
			$id = (int)$userData['ID'];
			if (!in_array($id, $userIdList, true))
			{
				continue;
			}
			...
			$users[$userData['ID']] = [
				...
				'COLLAB_USER' => $userData['COLLAB_USER'],
			];
		}
	...
	public static function GetAttendees($eventIdList = [], $checkDeleted = true)
	{
		...
		if (CCalendar::IsSocNet())
		{
			...
			if (!empty($eventIdList))
			{
				...
				{
					$query->addSelect('USER.UF_DEPARTMENT', 'UF_DEPARTMENT');
				}

				$entries = $query->exec()->fetchAll();
				...
				foreach ($entries as $entry)
				{
					...
					$userDepartment = !empty($entry['UF_DEPARTMENT']) ? $entry['UF_DEPARTMENT'] : [];
					CCalendar::SetUserDepartment($attendeeId, $userDepartment);
	...
	private static function getBusSafeChildEventIndexForMeetingSync(int $parentId, array $attendeeIds): array
	{
		$parentId = (int)$parentId;
		$attendeeIds = array_values(array_filter(
			array_unique(array_map('intval', $attendeeIds)),
			static fn($attendeeId) => $attendeeId > 0
		));

		if ($parentId <= 0 || empty($attendeeIds))
		{
			return [];
		}

		$childEventIndex = [];
		$requestedAttendeeIds = array_fill_keys($attendeeIds, true);
		$childEvents = Internals\EventTable::query()
			->setSelect([
				'USER_ID' => 'OWNER_ID',
				'ID',
				'PARENT_ID',
		...
		while ($entry = $childEvents->fetch())
		{
			if (
				$entry['CAL_TYPE'] !== Dictionary::CALENDAR_TYPE['user']
				|| $entry['ACTIVE'] !== 'Y'
				|| $entry['DELETED'] !== 'N'
			)
			{
			...
			if (
				$attendeeId <= 0
				|| !isset($requestedAttendeeIds[$attendeeId])
				|| isset($childEventIndex[$attendeeId])
			)
			{
				continue;
			}

			$status = trim((string)$entry['MEETING_STATUS']);
			if ($attendeeId === (int)$entry['MEETING_HOST'])
			{
				$status = 'H';
			}

			$childEventIndex[$attendeeId] = [
				'USER_ID' => $attendeeId,
				'EVENT_ID' => (int)$entry['ID'],
				'STATUS' => $status,
				'MEETING_STATUS' => $entry['MEETING_STATUS'],
				'MEETING_HOST' => (int)$entry['MEETING_HOST'],
			];
		}

		return $childEventIndex;
	}

	private static function mergeChildEventFallbackIntoCurrentAttendees(
		array $currentAttendeesIndex,
		array $childEventIndex,
		array $attendeeIds
	): array
	{
		foreach ($attendeeIds as $attendeeId)
		{
			...
			if (!empty($currentAttendeesIndex[$attendeeId]) || empty($childEventIndex[$attendeeId]))
			{
				continue;
			}

			$currentAttendeesIndex[$attendeeId] = $childEventIndex[$attendeeId];
		}

		return $currentAttendeesIndex;
	}

	private static function shouldUseBusSafeChildEventFallback(
		array $currentAttendeesIndex,
		array $missingCurrentAttendeeIds
	): bool
	{
		return empty($currentAttendeesIndex) && !empty($missingCurrentAttendeeIds);
	}

	public static function ApplyAccessRestrictions($event, $userId = null)
	{
		if (!$userId)
		{
		...
		{
			$sectId = (int)CCalendar::GetMeetingSection($userId);
		}

		$accessResult = self::checkEventAccessFromGetList($event, $sectId, $userId);
	...
	public static function ParseRecursion(&$res, $event, $params = [])
	{
		...
		if ($limitFromTS < (int)$event['DATE_FROM_TS_UTC'])
		{
			$limitFromTS = (int)$event['DATE_FROM_TS_UTC'];
		}

		$eventDateToTsUtc = (int)$event['DATE_TO_TS_UTC'];
		if (isset($rrule['COUNT']))
		{
			...
			$skipTime
			&& isset($rrule['UNTIL_TS'])
			&& $eventDateToTsUtc > (int)$rrule['UNTIL_TS']
		)
		{
			$eventDateToTsUtc = (int)$rrule['UNTIL_TS'];
		}
	...
	public static function ParseRRULE($rule = null)
	{
		...
		$res['UNTIL_TS'] = !empty($res['UNTIL']) && is_string($res['UNTIL'])
			? CCalendar::TimestampUTC($res['UNTIL'])
			: null
		;
	...
	public static function CheckEntryChanges($newFields = [], $currentFields = [])
	{
		...
		foreach ($fieldList as $fieldKey)
		{
			...
			else if ($fieldKey === 'DATE_FROM')
			{
				if (
					(
						!empty($newFields[$fieldKey])
						&& !empty($currentFields[$fieldKey])
	...
	private static function CreateChildEvents($parentId, $arFields, &$params, $changeFields)
	{
		...
		if (!$isNewEvent)
		{
			...
			foreach ($curAttendees as $user)
			{
				...
				$curAttendeeId = (int)$user['USER_ID'];

				$isUserCalendar = $arFields['CAL_TYPE'] === Dictionary::CALENDAR_TYPE['user'];
				$isHost = $curAttendeeId === (int)$arFields['MEETING_HOST'];
				$isOwner = $curAttendeeId === (int)$arFields['OWNER_ID'];

				if (
					(
						!$isHost
						|| (!$isOwner && $isUserCalendar)
					)
					&& (
						!$isOwner
						|| !$isUserCalendar
				...
				{
					$deletedAttendees[$user['USER_ID']] = $curAttendeeId;
					$involvedAttendees[] = $curAttendeeId;
				}
			}

			$missingCurrentAttendeeIds = array_filter(
				array_unique(array_map('intval', $attendees)),
				static fn($attendeeId) => $attendeeId > 0 && empty($currentAttendeesIndex[$attendeeId])
			);
			if (self::shouldUseBusSafeChildEventFallback($currentAttendeesIndex, $missingCurrentAttendeeIds))
			{
				$currentAttendeesIndex = self::mergeChildEventFallbackIntoCurrentAttendees(
					$currentAttendeesIndex,
					self::getBusSafeChildEventIndexForMeetingSync($parentId, $missingCurrentAttendeeIds),
					$missingCurrentAttendeeIds,
				);
			}
		}
		...
		$parentRecurrentEvents =
			!empty($params['arFields']['RECURRENCE_ID'])
				? self::getRecurrentEventDataByAttendees($params['arFields']['RECURRENCE_ID'])
				: []
		;
		...
		foreach ($attendees as $userKey)
		{
			...
			;
			self::clearCalendarUserCache($attendeeId);
			...
			)
			{
				...
				$childParams['arFields'] = [
					...$clonedParams['arFields'],
					'CAL_TYPE' => 'user',
					'PARENT_ID' => $parentId,
					'OWNER_ID' => $attendeeId,
					'CREATED_BY' => $attendeeId,
					'CREATED' => $arFields['DATE_CREATE'] ?? null,
					'MODIFIED' => $arFields['TIMESTAMP_X'] ?? null,
					'ACCESSIBILITY' => $arFields['ACCESSIBILITY'] ?? null,
					'MEETING' => $arFields['~MEETING'] ?? null,
					'TEXT_LOCATION' => CCalendar::GetTextLocation($arFields["LOCATION"] ?? null),
					'MEETING_STATUS' => $attendeeStatuses[$attendeeId]['status'] ?? null,
					'EVENT_TYPE' => $arFields['EVENT_TYPE'] ?? null,
				];
				...
				// If there is no parent recurrent event for user, then child event shouldn't be recurrent as well
				$ownerId = (int)($childParams['arFields']['OWNER_ID'] ?? 0);
				$parentRecurrentEvent = $parentRecurrentEvents[$ownerId] ?? [];

				if (empty($parentRecurrentEvent))
				{
					unset(
				...
				if (!empty($currentAttendeesIndex[$attendeeId]))
				{
					...
					elseif (
						(int)$arFields['MEETING_HOST'] !== $attendeeId
						&& ($currentAttendeesIndex[$attendeeId]['STATUS'] ?? null) === 'Y'
					)
					{
						// Reinvite explicitly resets an already-accepted attendee back to pending before sending a new invite.
						$resetStatus = true;
					}
				...
				else
				{
					...
					if ($parentRecurrentEvent)
					{
						$childParams['arFields']['DAV_XML_ID'] = $parentRecurrentEvent['DAV_XML_ID'] ?? null;
					}
					else
					{
					...
					)
					{
						$davParams = [
							...
							'instanceTz' => $parentRecurrentEvent['TZ_FROM'] ?? null,
							...
							'parentDateFrom' => $parentRecurrentEvent['DATE_FROM'] ?? null,
							'parentDateTo' => $parentRecurrentEvent['DATE_TO'] ?? null,
						];
				...
				)
				{
					...
					if (!self::$isAddIcalFailEmailError && !empty($email = self::getSenderEmailForIcal($arFields['MEETING'])))
					{
						$sender['EMAIL'] = $email;
					}
					else
				...
				if (
					...
					&& empty($childParams['arFields']['ORIGINAL_RECURSION_ID']) // to prevent adding on instance only
				)
				{
					$chat->AddUser(
						chatId: $chatId,
		...
		if (!$isNewEvent && !empty($deletedAttendees))
		{
			...
			foreach($deletedAttendees as $attendeeId)
			{
				...
				{
					self::clearCalendarUserCache($att["USER_ID"]);
	...
	public static function UpdateParentEventExDate($recurrenceId, $exDate, $attendeeIds)
	{
		...
		if (is_array($attendeeIds))
		{
			...
			{
				self::clearCalendarUserCache($id);
			}
		}
	...
	public static function Delete($params)
	{
		...
		if ($id)
		{
			...
			if ($entry)
			{
				...
				{
					self::clearCalendarUserCache($entry['OWNER_ID']);
				}
				...
				if (!empty($entry['IS_MEETING']))
				{
					...
					self::clearCalendarUserCache($userId);
					...
					{
						self::clearCalendarUserCache($chEvent["OWNER_ID"]);
						...
						)
						{
							...
							)
							{
								if (($eventLink instanceof Sharing\Link\EventLink))
								{
									...
									$chEvent['CAL_TYPE'] === Dictionary::CALENDAR_TYPE['user']
									&& !empty($chEvent["OWNER_ID"])
								)
								{
									\CCalendarNotify::Send([
										'mode' => 'cancel',
				...
				if ($params && is_array($params))
				{
					...
					$loggerContext['entityId'] = $id;
					$loggerContext['type'] = 'portal_delete';
					$loggerContext['userId'] = $userId;
					unset($loggerContext['Event']);

					ServiceLocator::getInstance()
						->get(RequestLogger::class)
						->debug('Delete event', $loggerContext)
					;
				}
	...
	public static function SetMeetingStatus($params)
	{
		...
		if ($event && $event['IS_MEETING'] && (int)$event['PARENT_ID'] > 0)
		{
			...
			if ($createdBy !== $userId)
			{
				self::clearCalendarUserCache($createdBy);
			}
		...
		(new \Bitrix\Calendar\Event\Event\AfterMeetingStatusChanged($eventId, $userId, $status))->emit();
	}
	...
	): void
	{
		if (in_array('COLOR', $changedFields, true) && isset($dbFields['COLOR']))
		{
			...
			$allChildEventAttendees = Internals\EventAttendeeTable::getList([
				'select' => ['ID'],
				'filter' => [
	...
	/**
	 * @deprecated
	 * @use self::cleanEventsWithDeletedParent instead
	 */
	...
	public static function cleanEventsWithDeletedParent(int $parentId): void
	{
		Internals\EventTable::updateByFilter(
			[
				'PARENT_ID' => $parentId,
			...
			if (!empty($event['ATTENDEE_LIST']) && is_array($event['ATTENDEE_LIST']))
			{
				foreach($event['ATTENDEE_LIST'] as $attendee)
				{
					if ($attendee['status'] === 'Y')
					{
						...
						self::clearCalendarUserCache($attendee["id"]);
		...
		if (!empty($entry))
		{
			...
			try
			{
				if (!empty($entry['UF_CRM_CAL_EVENT']) && Loader::includeModule('crm'))
				{
					...
					foreach ($uf as $item)
					{
						$title = self::getCrmElementTitle($item);
	...
	public static function getCrmElementTitle(string $item)
	{
		$crmElement = explode('_', $item);
		$type = $crmElement[0];
		$typeId = \CCrmOwnerType::ResolveID(\CCrmOwnerTypeAbbr::ResolveName($type));

		return \CCrmOwnerType::GetCaption($typeId, $crmElement[1]);
	}
	...
	public static function getEventForViewInterface($entryId, $params = [])
	{
		...
		$fromTs = self::getEventTimeStampForOrmFilter((string)$params['eventDate'], $timezoneOffset);
		$userDateFrom = \CCalendar::Date($fromTs);
		$eventDate = \CCalendar::Date($fromTs + $timezoneOffset, false);
		...
		if (
			...
			&& $entry[0]['RRULE']
			&& $entry[0]['EXDATE']
			&& in_array($eventDate, self::GetExDate($entry[0]['EXDATE']))
		)
		{
			$entry = self::GetList([
                'arFilter' => [
		...
		if (!$entry || !is_array($entry[0]) || !isset($entry[0]['ID']))
		{
			$entry = self::GetList([
				'arFilter' => [
					"ID" => $entryId,
		...
		if ($entry && is_array($entry[0]))
		{
			...
			if ($entry['IS_MEETING'] && (int)$entry['PARENT_ID'] !== (int)$entry['ID'])
			{
				...
				$parentFromTs = self::getEventTimeStampForOrmFilter($entry['DATE_FROM'], $timezoneOffset);
				$dateForParent = \CCalendar::Date($parentFromTs);

				$parentEntryList = self::GetList([
					'arFilter' => [
						'ID' => $parentId,
						'FROM_LIMIT' => $dateForParent,
						'TO_LIMIT' => $dateForParent,
						'DELETED' => false,
					],
					...$defaultParams,
				]);
	...
	public static function getEventForEditInterface($entryId, $params = [])
	{
		...
		if (is_array($entry) && $eventId !== $parentId)
		{
			$parentEntry = self::getEventForEditInterface($parentId, $params);
	...
	private static function checkRecurringRuleField(array &$arFields, int $toTs, $currentExDate): void
	{
		...
		)
		{
			...
			{
				$arFields['RRULE']['COUNT'] = self::limitRRuleCount($arFields['RRULE']['COUNT']);
			}
	...
	{
		$timezone = self::getRRuleCalculationTimezone($arFields);
		if ($timezone === null)
		{
			return self::calculateUntilForCountRRuleInCurrentTimezone($arFields, $offsetTs);
		}
	...
	private static function calculateUntilForCountRRuleInCurrentTimezone(array $arFields, int $offsetTs = 0): int
	{
		$rrule = self::ParseRRULE($arFields['RRULE']);
		$interval = (int)$rrule['INTERVAL'];
		$count = self::limitRRuleCount($rrule['COUNT']);
		...
		if ($rrule['FREQ'] === 'WEEKLY')
		{
			$byDay = CCalendar::normalizeRRuleByDay($rrule['BYDAY'] ?? null);

			if (empty($byDay) || $count <= 0)
			{
				return 0;
			}

			$interval = max(1, $interval);
			$startWeekTimestamp = mktime(
				0,
				0,
				0,
				(int)date('m', $fromTS),
				(int)date('d', $fromTS) - (((int)date('w', $fromTS) + 6) % 7),
				(int)date('Y', $fromTS)
			);
			$weekIndex = 0;
			$realCount = 0;
			$lastOccurrenceTimestamp = $fromTS;

			while ($realCount < $count)
			{
				$weekTimestamp = mktime(
					0,
					0,
					0,
					(int)date('m', $startWeekTimestamp),
					(int)date('d', $startWeekTimestamp) + $weekIndex * 7,
					(int)date('Y', $startWeekTimestamp)
				);

				for ($dayOffset = 0; $dayOffset < 7; $dayOffset++)
				{
					$weekDay = CCalendar::WeekDayByInd($dayOffset, false);
					if (!$weekDay || !isset($byDay[$weekDay]))
					{
						continue;
					}

					$candidateTimestamp = mktime(
						(int)date('H', $fromTS),
						(int)date('i', $fromTS),
						(int)date('s', $fromTS),
						(int)date('m', $weekTimestamp),
						(int)date('d', $weekTimestamp) + $dayOffset,
						(int)date('Y', $weekTimestamp)
					);

					if ($candidateTimestamp < $fromTS)
					{
						continue;
					}

					$realCount++;
					$lastOccurrenceTimestamp = $candidateTimestamp;
			...
			return $lastOccurrenceTimestamp + $length;
		}

		return 0;
	}

	private static function getRRuleCalculationTimezone(array $arFields): ?string
	{
		$timezone = $arFields['TZ_FROM'] ?? $arFields['TZ_TO'] ?? null;
	...
	private static function limitRRuleCount($count): int
	{
		$count = (int)$count;

		return $count > self::MAX_RRULE_COUNT ? self::MAX_RRULE_COUNT : $count;
	}
	...
	{
		// @todo Remove with deprecated agent \Bitrix\Calendar\Core\Queue\Agent\EventDelayedSyncAgent
	...
	public static function getEventModelForPermissionCheck(int $eventId, array $event = [], int $userId = 0): EventModel
	{
		...
		if (
			...
			&& (
				empty($event)
				|| ((int)($event['ID'] ?? 0) !== $eventId)
			)
		)
		{
			$currentEventResult = self::GetList([
				'arFilter' => [
					'ID' => $eventId,
					'DELETED' => 'N',
				],
	...
	private static function getOpenEventSection(?int $userId = null): ?Section
	{
		if (!(OpenEvents\Feature::getInstance()->isAvailable($userId)))
		{
			return null;
		}

		if (self::$openEventSection !== false)
		{
			return self::$openEventSection;
		}
		...
		self::$openEventSection = $openEventSection;
	 ...
	 */
	private static function checkEventAccessFromGetList(array $event, int $sectionId, int $userId): array
	{
		if (isset(self::$getListAccessCheck[$sectionId]))
		{
			return self::$getListAccessCheck[$sectionId];
		}

		$eventModel = EventModel::createFromArray($event);
		$eventModel->setSectionId($sectionId);
	...
	): array
	{
		...
		foreach ($attendees as $attendee)
		{
			...
			if (
				($fields['CAL_TYPE'] ?? null) === 'user'
				&& (int)($fields['PARENT_ID'] ?? 0) === (int)($fields['ID'] ?? 0)
				&& (int)($fields['OWNER_ID'] ?? 0) === $attendeeId
			)
			{
				$meetingStatus = 'H';
			}
			elseif ((int)($fields['CREATED_BY'] ?? 0) === $attendeeId)
			{
				$meetingStatus = 'Y';
			}
			...
			if (
				...
				&& ($params['sendInvitesToDeclined'] ?? false)
				&& $meetingStatus === 'N'
			)
			{
				$meetingStatus = 'Q';
	...
	private static function getRecurrentEventDataByAttendees(int $recurrentId): array
	{
		$result = [];
		...
		foreach ($rows as $row)
		{
			$result[(int)$row['OWNER_ID']] = $row;
		}

		return $result;
	}

	private static function clearCalendarUserCache(mixed $userId = null): void
	{
	...
	private static function getEventTimeStampForOrmFilter(string $eventDate, int $timezoneOffset)
	{
		$timestamp = $eventDate
			? \CCalendar::Timestamp($eventDate, true, false) - $timezoneOffset
			: 0
		;

		// event date is 1970.01.01 or not got
		if ($timestamp < 86401)
		{
			$timestamp = 0;
		}
