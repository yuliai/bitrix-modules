...
use Bitrix\Rest\Public\Command\Application\InstallLocalAppCommand;
use Bitrix\Rest\Public\Command\Application\UninstallLocalAppCommand;
...
final class Local extends RestController
{
	use CommandRunner;
	...
	#[Title(new LocalizableMessage(code: 'REST_INFRASTRUCTURE_REST_CONTROLLER_APPLICATION_LOCAL_INSTALLACTION_TITLE', phraseSrcFile: __FILE__))]
	#[Description('
		Request example:
		{
	...
	): InstallAppResponse
	{
		...
		$result = $this->runCommand(new InstallLocalAppCommand(
			userId: $userId,
	...
	#[Title(new LocalizableMessage(code: 'REST_INFRASTRUCTURE_REST_CONTROLLER_APPLICATION_LOCAL_UNINSTALLACTION_TITLE', phraseSrcFile: __FILE__))]
	#[Description('
		Request example:
		{
	...
	{
		$this->runCommand(new UninstallLocalAppCommand(
			userId: (int)$this->getCurrentUser()->getId(),
			clientId: $clientId,
		));
