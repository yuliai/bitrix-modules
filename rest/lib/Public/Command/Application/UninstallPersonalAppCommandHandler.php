...
class UninstallPersonalAppCommandHandler
{
	 ...
	 */
	public function __invoke(UninstallPersonalAppCommand $command): bool
	{
		...
		if ($app->getOrigin() !== AppOrigin::Local)
		{
			throw new AccessDeniedException(
				'Application is not a local application',
			);
		}
		...
		{
			throw new AccessDeniedException(
				'Application is not a personal application',
			);
		}

		$accessChecker = new AppAccessChecker($command->userId);
		if (!$accessChecker->canUninstallPersonal($app))
		{
			throw new AccessDeniedException(
				'User does not have rights to uninstall this personal application',
			);
		}
