...
use Bitrix\MessageService\Sender\Sms\SmsLineBy;
...
final class MobileMarketing extends Provider
{
	...
	public function createConnectionsSliderSections(array $senders): array
	{
		...
		foreach ($editorViewChannels as $editorViewChannel)
		{
			...
			$iconPath = $isConnected
				? '/bitrix/components/bitrix/messageservice.connections/images/sms-mobilemarketing-connected.svg'
				: '/bitrix/components/bitrix/messageservice.connections/images/sms-mobilemarketing-not-connected.svg';
	...
	public function createEditorViewChannels(array $senders): array
	{
		$usedSenders = array_filter(
			$senders,
			static fn(\Bitrix\MessageService\Sender\Base $sender): bool => (string)$sender->getId() === SmsLineBy::ID,
		);
