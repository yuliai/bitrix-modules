...
function AddFont($family, $style='', $file='', $uni=false)
{
	...
	if ($uni) {
		...
		else { $ttffilename = $this->_getfontpath().'/'.$file ; }
		$unifilename = $this->_getfontpath().'/'.mb_strtolower(mb_substr($file,0,(mb_strpos($file,'.',0,'8bit')),'8bit'));
		$name = '';
		$originalsize = 0;
		...
		if (!isset($type) ||  !isset($name) || $originalsize != $ttfstat['size']) {
			...
			$s.='$fontkey=\''.$fontkey."';\n";
			$s.="?>";
			if (is_writable(dirname($this->_getfontpath().'/'.'x'))) {
				$fh = fopen($unifilename.'.mtx.php',"w");
				fwrite($fh,$s,mb_strlen($s,'8bit'));
				fclose($fh);
				$fh = fopen($unifilename.'.cw.dat',"wb");
				fwrite($fh,$cw,mb_strlen($cw,'8bit'));
				fclose($fh);
...
function SetFont($family, $style='', $size=0)
{
	...
	if($family=='')
		...
		$family = mb_strtolower($family);
	$style = mb_strtoupper($style);
	if(mb_strpos($style,'U',0,'8bit')!==false)
	{
		$this->underline = true;
...
function Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='')
{
	...
	if(is_string($border))
	{
		$x = $this->x;
		$y = $this->y;
		if(mb_strpos($border,'L',0,'8bit')!==false)
			$s .= sprintf('%.2F %.2F m %.2F %.2F l S ',$x*$k,($this->h-$y)*$k,$x*$k,($this->h-($y+$h))*$k);
		if(mb_strpos($border,'T',0,'8bit')!==false)
			$s .= sprintf('%.2F %.2F m %.2F %.2F l S ',$x*$k,($this->h-$y)*$k,($x+$w)*$k,($this->h-$y)*$k);
		if(mb_strpos($border,'R',0,'8bit')!==false)
			$s .= sprintf('%.2F %.2F m %.2F %.2F l S ',($x+$w)*$k,($this->h-$y)*$k,($x+$w)*$k,($this->h-($y+$h))*$k);
		if(mb_strpos($border,'B',0,'8bit')!==false)
...
function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false)
{
	...
	if($border)
	{
		...
		else
		{
			...
			if(mb_strpos($border,'R',0,'8bit')!==false)
				$b2 .= 'R';
			$b = (mb_strpos($border,'T',0,'8bit')!==false) ? $b2.'T' : $b2;
		}
	}
	$sep = -1;
	$i = 0;
	...
	while($i<$nb)
	{
		...
		if($c=="\n")
		{
			...
			if ($this->unifontSubset) {
				...
				$this->Cell($w,$h,mb_substr($s,$j,$i-$j,'8bit'),$b,2,$align,$fill);
			}
			$i++;
			$sep = -1;
			$j = $i;
		...
		if($l>$wmax)
		{
			...
			else
			{
				...
				if ($this->unifontSubset) {
					...
					$this->Cell($w,$h,mb_substr($s,$j,$sep-$j,'8bit'),$b,2,$align,$fill);
				}
				$i = $sep+1;
			}
			$sep = -1;
	...
	if($border && mb_strpos($border,'B',0,'8bit')!==false)
		$b .= 'B';
	if ($this->unifontSubset) {
		$this->Cell($w,$h,mb_substr($s,$j,$i-$j,'UTF-8'),$b,2,$align,$fill);
	}
...
function Write($h, $txt, $link='')
{
	...
	while($i<$nb)
	{
		...
		if($c=="\n")
		{
			...
			if ($this->unifontSubset) {
				...
				$this->Cell($w,$h,mb_substr($s,$j,$i-$j,'8bit'),0,2,'',0,$link);
			}
			$i++;
			$sep = -1;
			$j = $i;
		...
		if($l>$wmax)
		{
			...
			else
			{
				if ($this->unifontSubset) {
					...
					$this->Cell($w,$h,mb_substr($s,$j,$sep-$j,'8bit'),0,2,'',0,$link);
				}
				$i = $sep+1;
			}
			$sep = -1;
	...
	if($i!=$j) {
		if ($this->unifontSubset) {
			...
			$this->Cell($l,$h,mb_substr($s,$j,mb_strlen($s,'8bit'),'8bit'),0,0,'',0,$link);
		}
	}
}
...
function Image($file, $x=null, $y=null, $w=0, $h=0, $type='', $link='')
{
	...
	if(!isset($this->images[$file]))
	{
		...
		if($type=='')
		{
			$pos = mb_strrpos($file,'.',0,'8bit');
			if(!$pos)
				$this->Error('Image file has no extension and no type was specified: '.$file);
			$type = mb_substr($file,$pos+1,mb_strlen($file,'8bit'),'8bit');
		}
...
function Output($name='', $dest='', $utfName  = '')
{
	...
	switch($dest)
	{
		...
		case 'F':
			...
			fwrite($f,$this->buffer,mb_strlen($this->buffer,'8bit'));
			fclose($f);
			break;
		case 'S':
			// Return as a string
...
function _dounderline($x, $y, $txt)
{
	...
	$w = $this->GetStringWidth($txt)+$this->ws*mb_substr_count($txt,' ','8bit');
...
function _parsepngstream($f, $file)
{
	...
	do
	{
		...
		elseif($type=='tRNS')
		{
			...
			elseif($ct==2)
				$trns = array(ord(mb_substr($t,1,1,'8bit')), ord(mb_substr($t,3,1,'8bit')), ord(mb_substr($t,5,1,'8bit')));
			else
			{
				$pos = mb_strpos($t,chr(0),0,'8bit');
				if($pos!==false)
					$trns = array($pos);
			}
			$this->_readstream($f,4);
...
function _newobj()
{
	// Begin a new object
	$this->n++;
	$this->offsets[$this->n] = mb_strlen($this->buffer,'8bit');
...
function _putpages()
{
	...
	for($n=1;$n<=$nb;$n++)
	{
		...
		$this->_newobj();
		$this->_out('<<'.$filter.'/Length '.mb_strlen($p,'8bit').'>>');
		$this->_putstream($p);
		$this->_out('endobj');
	}
	// Pages root
	$this->offsets[1] = mb_strlen($this->buffer,'8bit');
	$this->_out('1 0 obj');
...
function _putfonts()
{
	...
	foreach($this->FontFiles as $file=>$info)
	{
		if (!isset($info['type']) || $info['type']!='TTF') {
			...
			while(!feof($f))
				$font.=fread($f,8192);
			fclose($f);
			$compressed=(mb_substr($file,-2,mb_strlen($file,'8bit'),'8bit')=='.z');
			if(!$compressed && isset($info['length2']))
			{
				...
				if($header)
				{
					...
					$font=mb_substr($font,6,mb_strlen($font,'8bit'),'8bit');
				}
				if($header && ord($font[$info['length1']])==128)
				{
					// Strip second binary header
					$font=mb_substr($font,0,$info['length1'],'8bit').mb_substr($font,$info['length1']+6,mb_strlen($font,'8bit'),'8bit');
				}
			}
			$this->_out('<</Length '.mb_strlen($font,'8bit'));
			if($compressed)
	...
	foreach($this->fonts as $k=>$font)
	{
		...
		// TrueType embedded SUBSETS or FULL
			...
			$ttfontsize = mb_strlen($ttfontstream,'8bit');
			...
			$toUni .= "end\n";
			$toUni .= "end";
			$this->_out('<</Length '.(mb_strlen($toUni,'8bit')).'>>');
			...
			$this->_newobj();
			$this->_out('<</Length '.mb_strlen($cidtogidmap,'8bit').'');
			$this->_out('/Filter /FlateDecode');
			$this->_out('>>');
	...
	for ($cid=$startcid; $cid<$cwlen; $cid++) {
		if ($cid==128 && (!file_exists($font['unifilename'].'.cw127.php'))) {
			if (is_writable(dirname($this->_getfontpath().'/x'))) {
				...
				$cw127.="?>";
				fwrite($fh,$cw127,mb_strlen($cw127,'8bit'));
				fclose($fh);
			}
		}
...
function _putimage(&$info)
{
	...
	if($info['cs']=='Indexed')
		$this->_out('/ColorSpace [/Indexed /DeviceRGB '.(mb_strlen($info['pal'],'8bit')/3-1).' '.($this->n+1).' 0 R]');
	else
	{
	...
	$this->_out('/Length '.mb_strlen($info['data'],'8bit').'>>');
	...
	if($info['cs']=='Indexed')
	{
		...
		$this->_newobj();
		$this->_out('<<'.$filter.'/Length '.mb_strlen($pal,'8bit').'>>');
		$this->_putstream($pal);
		$this->_out('endobj');
	}
}
...
function _putresources()
{
	...
	$this->_putimages();
	// Resource dictionary
	$this->offsets[2] = mb_strlen($this->buffer,'8bit');
	$this->_out('2 0 obj');
	$this->_out('<<');
