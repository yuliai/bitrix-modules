...
class LeadUserField
{
	...
	public static function onBIConnectorDataSources(\Bitrix\Main\Event $event)
	{
		...
		if (!empty($eventTableName) && $eventTableName !== 'crm_lead_uf')
		{
			return;
		}
		...
		foreach ($result['crm_lead_uf']['FIELDS'] as $fieldCode => &$fieldInfo)
		{
			...
			{
				$fieldInfo['FIELD_DESCRIPTION_FULL'] = !empty($messages['CRM_BIC_LEAD_UF_FIELD_' . $fieldCode . '_FULL']) ? $messages['CRM_BIC_LEAD_UF_FIELD_' . $fieldCode . '_FULL'] : '';
			}
		}
		unset($fieldInfo);
	}
