...
class LeadProductRow
{
	...
	public static function onBIConnectorDataSources(\Bitrix\Main\Event $event)
	{
		...
		if (!empty($eventTableName) && $eventTableName !== 'crm_lead_product_row')
		{
			return;
		}
		...
		{
			$fieldInfo['FIELD_DESCRIPTION'] = !empty($messages['CRM_BIC_LEAD_PRODUCT_ROW_FIELD_' . $fieldCode]) ? $messages['CRM_BIC_LEAD_PRODUCT_ROW_FIELD_' . $fieldCode] : $fieldCode;
			if (!$fieldInfo['FIELD_DESCRIPTION'])
			{
				$fieldInfo['FIELD_DESCRIPTION'] = $fieldCode;
			}

			$fieldInfo['FIELD_DESCRIPTION_FULL'] = !empty($messages['CRM_BIC_LEAD_PRODUCT_ROW_FIELD_' . $fieldCode . '_FULL']) ? $messages['CRM_BIC_LEAD_PRODUCT_ROW_FIELD_' . $fieldCode . '_FULL'] : '';
		}
		unset($fieldInfo);
	}
}
