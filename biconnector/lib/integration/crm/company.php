...
class Company
{
	...
	public static function onBIConnectorDataSources(\Bitrix\Main\Event $event)
	{
		...
		$isPgsql = $connection instanceof \Bitrix\Main\DB\PgsqlConnection;
		...
		$result['crm_company'] = [
			...
			'FIELDS' => [
				...
				'COMPANY_TYPE' => [
					'IS_METRIC' => 'N', // 'Y'
					'FIELD_NAME' => 'CASE WHEN C.COMPANY_TYPE is null THEN null ELSE concat_ws(\' \', ' . $helper->getConcatFunction("'['", 'C.COMPANY_TYPE', "']'") . ', nullif(S.NAME, \'\')) END',
					'FIELD_TYPE' => 'string',
					'TABLE_ALIAS' => 'S',
					'JOIN' => 'INNER JOIN b_crm_status S ON S.ENTITY_ID = \'COMPANY_TYPE\' and S.STATUS_ID = C.COMPANY_TYPE',
					'LEFT_JOIN' => 'LEFT JOIN b_crm_status S ON S.ENTITY_ID = \'COMPANY_TYPE\' and S.STATUS_ID = C.COMPANY_TYPE',
				...
				'INDUSTRY' => [
					'IS_METRIC' => 'N', // 'Y'
					'FIELD_NAME' => 'CASE WHEN C.INDUSTRY is null THEN null ELSE concat_ws(\' \', ' . $helper->getConcatFunction("'['", 'C.INDUSTRY', "']'") . ', nullif(I.NAME, \'\')) END',
					'FIELD_TYPE' => 'string',
					'TABLE_ALIAS' => 'I',
					'JOIN' => 'INNER JOIN b_crm_status I ON I.ENTITY_ID = \'INDUSTRY\' and I.STATUS_ID = C.INDUSTRY',
					'LEFT_JOIN' => 'LEFT JOIN b_crm_status I ON I.ENTITY_ID = \'INDUSTRY\' and I.STATUS_ID = C.INDUSTRY',
				...
				'REVENUE' => [
					'IS_METRIC' => 'N', // 'Y'
					'FIELD_NAME' => $isPgsql
						? "(CASE WHEN C.REVENUE ~ '^-?[0-9]+(\\.[0-9]+)?$' THEN C.REVENUE ELSE NULL END)::double precision"
						: '(C.REVENUE + 0.0)',
					'FIELD_TYPE' => 'double',
				],
		...
		$result['crm_company']['TABLE_DESCRIPTION_FULL'] = $messages['CRM_BIC_COMPANY_TABLE_DESCRIPTION_FULL'] ?? '';
		foreach ($result['crm_company']['FIELDS'] as $fieldCode => &$fieldInfo)
		{
			$fieldInfo['FIELD_DESCRIPTION'] = !empty($messages['CRM_BIC_COMPANY_FIELD_' . $fieldCode]) ? $messages['CRM_BIC_COMPANY_FIELD_' . $fieldCode] : $fieldCode;
			if (!$fieldInfo['FIELD_DESCRIPTION'])
			{
				$fieldInfo['FIELD_DESCRIPTION'] = $fieldCode;
			}

			$fieldInfo['FIELD_DESCRIPTION_FULL'] = !empty($messages['CRM_BIC_COMPANY_FIELD_' . $fieldCode . '_FULL']) ? $messages['CRM_BIC_COMPANY_FIELD_' . $fieldCode . '_FULL'] : '';
		}
		unset($fieldInfo);
	}
}
