...
use Bitrix\Im\V2\Pull\Event\RecentStickerDelete;
use Bitrix\Im\V2\Pull\Event\RecentStickerDeleteAll;
...
class RecentSticker
{
	...
	public function get(int $limit = 12): array
	{
		...
		return (new StickerService())->getStickerByRecent($recentItems);
	}
	...
	public function delete(int $stickerId, int $packId, string $packType): void
	{
		...
		(new RecentStickerDelete($stickerId, $packId, $packType))->send();
	}

	public function deleteAll(): void
	{
		StickerRecentTable::deleteBatch(['=USER_ID' => $this->getContext()->getUserId()]);
		(new RecentStickerDeleteAll())->send();
	}
