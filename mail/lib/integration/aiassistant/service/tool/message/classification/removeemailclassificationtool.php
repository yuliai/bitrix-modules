...
class RemoveEmailClassificationTool extends ToolContract
{
	public const ACTION_NAME = 'remove_email_classification';
	...
	{
		return
			"Removes a specific classification label from an email (no-op if not set). "
	...
	public function getInputSchema(): array
	{
		return [
			...
			'properties' => [
				'messageId' => [
					'type' => 'integer',
					'description' => 'Identifier of the email to remove classification from.',
					'minimum' => 1,
				],
				'label' => [
					'type' => 'string',
					'enum' => $this->allowedLabels(),
					'description' => 'Classification label to remove.',
				],
			],
	...
	protected function executeStructured(int $userId, ...$args): array
	{
		...
		if (!$this->classificationService->removeLabelByUser($userId, $messageId, $label)->isSuccess())
		{
