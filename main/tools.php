...
function _FormatDateMessage($value, $messages, ?string $languageId = null)
{
	...
	if ($val == 0)
	{
		return Main\Localization\Loc::getMessage($messages["0"], ["#VALUE#" => $value], $languageId);
	}
	if ($val == 1)
	{
		return Main\Localization\Loc::getMessage($messages["1"], ["#VALUE#" => $value], $languageId);
	}
	if ($val >= 10 && $val <= 20)
	{
		return Main\Localization\Loc::getMessage($messages["10_20"], ["#VALUE#" => $value], $languageId);
	}
	if ($dec == 1)
	{
		return Main\Localization\Loc::getMessage($messages["MOD_1"], ["#VALUE#" => $value], $languageId);
	}
	if (2 <= $dec && $dec <= 4)
	{
		return Main\Localization\Loc::getMessage($messages["MOD_2_4"], ["#VALUE#" => $value], $languageId);
	}

	return Main\Localization\Loc::getMessage($messages["MOD_OTHER"], ["#VALUE#" => $value], $languageId);
}
