...
$arFormsList = ["authorize", "forgot_password", "change_password", "otp"];
if (!in_array($inc_file, $arFormsList))
{
	$inc_file = $arFormsList[0];
}

function dump_post_var($vname, $vvalue, $var_stack = [])
{
	if (is_array($vvalue))
	{
		$str = "";
		...
		{
			$str .= ($str == "" ? '' : '&') . dump_post_var($key, $value, array_merge($var_stack, [$vname]));
		}
		return $str;
	}
	else
...
if (CModule::IncludeModule('security') && Otp::isPushPossible())
{
	...
	{
		$pullConfig = PushOtp::getPullConfig();
	}
}
...
if (!CMain::IsHTTPS() && COption::GetOptionString('main', 'use_encrypted_auth', 'N') == 'Y')
{
	...
	{
		$sec->SetKeys($arKeys);
		$sec->AddToForm('form_auth', ['USER_PASSWORD', 'USER_CONFIRM_PASSWORD']);
		$bSecure = true;
	}
}
?>
	<script>
		BX.message({
			...
			'admin_push_otp_error': '<?= GetMessageJS('admin_push_otp_error')?>'
		});
				...
				<div class="login-header">
					<a href="/" class="login-logo">
						<span class="login-logo-img"></span><span
							class="login-logo-text"><?= $_SERVER["SERVER_NAME"] ?></span>
					</a>
					<div class="login-language-btn-wrap">
						<div class="login-language-btn" id="login_lang_button"><?= $arLangButton['TEXT'] ?></div>
					</div>
				</div>
	...
	<div id="login_variants" style="display: none;">
		...
		<div id="change_password_message" class="login-popup-wrap login-popup-ifo-wrap">
			<div class="login-popup">
				...
				<a class="login-popup-link" href="javascript:void(0)"
				   onclick="BX.adminLogin.toggleAuthForm('authorize')"><?= GetMessage('AUTH_GOTO_AUTH_FORM') ?></a>
			</div>
		</div>
