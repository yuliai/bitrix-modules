...
function CheckFilterDates($date1, $date2, &$date1_wrong, &$date2_wrong, &$date2_less_date1)
{
	...
	if ($date1_wrong != "Y" && $date2_wrong != "Y" && $date1 <> '' && $date2 <> '' && $DB->CompareDates($date2, $date1) < 0)
	{
		$date2_less_date1 = "Y";
	}
}

function InitFilterEx($arName, $varName, $action = "set", $useSession = true, $FilterLogic = "FILTER_logic")
{
	$session = Application::getInstance()->getSession();

	if (isset($session["SESS_ADMIN"][$varName]) && is_array($session["SESS_ADMIN"][$varName]))
	{
		$FILTER = $session["SESS_ADMIN"][$varName];
	}
	else
	{
		$FILTER = [];
	...
	if ($action == "set")
	{
		$FILTER[$FilterLogic] = $_REQUEST[$FilterLogic] ?? '';
	}
	...
	for ($i = 0, $n = count($arName); $i < $n; $i++)
	{
		...
		if ($action == "set")
		{
			...
			{
				$FILTER[$period] = $_REQUEST[$period] ?? null;
				$$period = $FILTER[$period];
			}

			if (isset($_REQUEST[$direction]) || isset($FILTER[$direction]))
			{
				$FILTER[$direction] = $_REQUEST[$direction] ?? null;
				$$direction = $FILTER[$direction];
			}

			if (isset($_REQUEST[$bdays]) || isset($FILTER[$bdays]))
			{
				$FILTER[$bdays] = $_REQUEST[$bdays] ?? null;
				if ((string)$_REQUEST[$bdays] <> '' && $_REQUEST[$bdays] != "NOT_REF")
				{
					$FILTER[$name] = GetTime(time() - 86400 * intval($FILTER[$bdays]));
					$$name = $FILTER[$name];
				}
			}
			...
			if ($useSession)
			{
				$session["SESS_ADMIN"][$varName] = $FILTER;
			}
		}
		else
		{
...
function GetFilterHiddens($var = "filter_", $button = ["filter" => "Y", "set_filter" => "Y"])
{
	$res = '';
...
function GetFilterParams($var = "filter_", $bDoHtmlEncode = true, $button = ["filter" => "Y", "set_filter" => "Y"])
{
	$arrVars = []; // массив имен переменных фильтра
...
function GetFilterQuery($field, $val, $procent = "Y", $ex_sep = [], $clob = "N", $div_fields = "Y", $clob_upper = "N")
{
	global $strError;
...
function GetFilterSqlSearch($arSqlSearch = [], $FilterLogic = "FILTER_logic")
{
	$var = $FilterLogic . "_reset";
...
function InitSorting($Path = false, $sByVar = "by", $sOrderVar = "order")
{
	...
	if (!empty($_REQUEST[$sByVar]))
	{
		$session["SESS_SORT_BY"][$md5Path] = $_REQUEST[$sByVar];
		$$sByVar = $_REQUEST[$sByVar];
	}
	else
	{
		$$sByVar = $session["SESS_SORT_BY"][$md5Path] ?? '';
	}

	if (!empty($_REQUEST[$sOrderVar]))
	{
		$session["SESS_SORT_ORDER"][$md5Path] = $_REQUEST[$sOrderVar];
		$$sOrderVar = $_REQUEST[$sOrderVar];
	}
	else
	{
		$$sOrderVar = $session["SESS_SORT_ORDER"][$md5Path] ?? '';
	}
