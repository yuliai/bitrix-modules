...
class CTasksReportHelper extends CReportHelper
{
	...
	protected static function prepareUFInfo()
	{
		...
		foreach (self::$arUFId as $ufId)
		{
			...
			if (is_array($arUserFields) && count($arUserFields) > 0)
			{
				...
				{
					if (
						isset($field['FIELD_NAME']) && \Bitrix\Tasks\Util\UserField::isUFKey($field['FIELD_NAME'])
						/*&& (!isset($field['MULTIPLE']) || $field['MULTIPLE'] !== 'Y')*/
						&& isset($field['USER_TYPE_ID'])
						&& in_array($field['USER_TYPE_ID'], $allowedUserTypes, true)
					)
					{
	...
	protected static function prepareUFEnumerations($usedUFMap = null)
	{
		...
		if (is_array($ufInfo))
		{
			foreach ($ufInfo as $entityId => $fieldList)
			{
				...
				{
					if (
						is_array($field) && isset($field['USER_TYPE_ID']) && $field['USER_TYPE_ID'] === 'enumeration'
						&& isset($field['ENTITY_ID'])
						&& strval($field['ENTITY_ID']) <> ''
						...
						&& is_array($field['USER_TYPE'])
						&& isset($field['USER_TYPE']['CLASS_NAME'])
						&& !empty($field['USER_TYPE']['CLASS_NAME'])
						&& is_callable([$field['USER_TYPE']['CLASS_NAME'], 'GetList'])
					)
					{
						self::$ufEnumerations[$field['ENTITY_ID']][$field['FIELD_NAME']] = [];
						$rsEnum = call_user_func_array([$field['USER_TYPE']['CLASS_NAME'], 'GetList'], [$field]);
	...
	public static function getColumnList()
	{
		...
		{
			if (
				isset(self::$ufInfo['TASKS_TASK']) && is_array(self::$ufInfo['TASKS_TASK'])
			...
			)
			{
				...
				{
					if (
						($uf['USER_TYPE_ID'] !== 'datetime' && $uf['USER_TYPE_ID'] !== 'boolean')
						|| $uf['MULTIPLE'] === 'Y'
						|| mb_substr($ufKey, -mb_strlen(self::UF_DATETIME_SHORT_POSTFIX))
						=== self::UF_DATETIME_SHORT_POSTFIX
	...
	public static function setRuntimeFields(\Bitrix\Main\Entity\Base $entity, $sqlTimeInterval)
	{
		...
		$entity->addField([ // in minutes (deprecated, use TIME_SPENT_IN_LOGS_FOR_PERIOD, which is in seconds, that works correctly in expressions)
			'data_type' => 'integer',
			'expression' => [
		...
		$entity->addField([ // in seconds
			'data_type' => 'integer',
			'expression' => [
				'(SELECT  SUM(CASE WHEN '
				. $sqlHelper->getDatetimeToDateFunction('CREATED_DATE')
				. ' '
				. $sqlTimeInterval
				.
	...
	{
		return array_merge(parent::getCalcVariations(), [
			'IS_OVERDUE_PRCNT' => [],
			'IS_MARKED_PRCNT' => [],
			'IS_EFFECTIVE_PRCNT' => [],
			'TAGS' => [],
			'Member:TASK_COWORKED.USER.SHORT_NAME' => [
				'COUNT_DISTINCT',
				'GROUP_CONCAT',
			],
		]);
		...
		&$runtime = null)
	{
		...
		// owner permission
		if (
			isset($_GET['select_my_tasks'])
			|| (!isset($_GET['select_my_tasks'])
				&& !isset($_GET['select_depts_tasks'])
				&& !isset($_GET['select_group_tasks']))
		)
		{
			$runtime['IS_TASK_COWORKER'] = [
				'data_type' => 'integer',
				'expression' => [
					"(CASE WHEN EXISTS("
		...
		{
			$allowedGroups = \Bitrix\Tasks\Integration\SocialNetwork\Group::getIdsByAllowedAction('view_all', true,
				\Bitrix\Tasks\Util\User::getId());
			$permFilter[] = ['=GROUP_ID' => $allowedGroups];
		}
				...
				elseif ($k !== 'LOGIC' && !is_numeric($k))
				{
					...
					if (preg_match('/^(>=|<=|=|>|<|!)?(.*' . $moneyFieldRegExp . ')$/', $k, $matches))
					{
						if (is_string($v) && $v !== '')
						{
							...
							if (
								is_array($valueParts) && isset($valueParts[0])
								&& is_string($valueParts[0])
								&& $valueParts[0] !== ''
							)
							{
	...
	public static function formatResultValue($k, &$v, &$row, &$cInfo, $total, &$customChartValue = null)
	{
		...
		elseif ($k == 'DESCRIPTION_TR')
		{
			...
			$v = htmlspecialcharsbx(str_replace("\x0D", ' ',
				str_replace("\x0A", ' ', PrepareTxtForEmail(strip_tags($v)))));
		}
		else
		{
	...
	public static function getDefaultReports()
	{
		...
		foreach ($reports as $version => &$vreports)
		{
			foreach ($vreports as $num => &$report)
			{
				...
				elseif ($version === '14.0.10' && $report['mark_default'] === 7)
				{
					...
					if ($ufDataType === 'boolean' && $arUF['ufInfo']['MULTIPLE'] !== 'Y')
					{
						...
						$dateFields[] = [
							'def' => [
								'data_type' => 'boolean',
								'expression' => [
									$sqlHelper->getIsNullFunction('%s', 0),
									$arUF['ufInfo']['FIELD_NAME'],
								],
							],
	...
	public static function appendMoneyUserFieldsAsSeparated(\Bitrix\Main\Entity\Base $entity)
	{
		...
		{
			if (
				in_array($field->getName(), ['LEAD_BY', 'COMPANY_BY', 'CONTACT_BY'], true)
				&& $field instanceof Bitrix\Main\Entity\ReferenceField
			)
			{
				self::appendMoneyUserFieldsAsSeparated($field->getRefEntity());
			}
			elseif ($field instanceof Bitrix\Main\Entity\ExpressionField)
			{
				$arUF = self::detectUserField($field);
				if ($arUF['isUF'])
				{
					...
					if ($ufDataType === 'money')
					{
						...
						$moneyFields[] = [
							'def' => [
								'data_type' => 'float',
								'expression' => [
									"({$helper->getIsNullFunction($moneyExpression, 0)})",
									$fieldName,
									$fieldName,
									$fieldName,
									$fieldName,
						...
						$currencyExpression = "IF(LOCATE('|', %s) > 0, SUBSTR(%s, LOCATE('|', %s) + 1), NULL)";
						$moneyFields[] = [
							'def' => [
								'data_type' => 'string',
								'expression' => [
									"({$helper->getIsNullFunction($currencyExpression, '')})",
									$fieldName,
									$fieldName,
									$fieldName,
								],
	...
	public static function getFDMsMultipleTrimmedDateTime()
	{
		return [
			[__CLASS__, 'fdmMultipleTrimmed'],
			[__CLASS__, 'fdmMultipleTrimmedDateTime'],
		];
	}
