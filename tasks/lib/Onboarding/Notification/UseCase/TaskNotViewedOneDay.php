...
use Bitrix\Tasks\Internals\Notification\Strategy\Default\TaskResponsibleReminderStrategy;
use Bitrix\Tasks\Internals\Notification\UseCase\AbstractCase;
use ReflectionClass;

class TaskNotViewedOneDay extends AbstractCase
{
		...
		foreach ($this->providers as $provider)
		{
			...
			foreach ($recipients as $recipient)
			{
				$metadata = new Metadata(
					...
					EntityOperation::NOT_VIEWED_ONE_DAY,
					[
						'task' => $this->task,
	...
	{
		return (new ReflectionClass(TaskResponsibleReminderStrategy::class))->getShortName();
	}
}
