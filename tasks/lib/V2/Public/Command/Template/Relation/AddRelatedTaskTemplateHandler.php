...
class AddRelatedTaskTemplateHandler
{
	...
	public function __invoke(AddRelatedTaskTemplateCommand $command): Template
	{
		return $this->consistencyResolver->resolve('template.related.add')->wrap(
			fn (): Template => $this->relatedTaskTemplateService->add($command->templateId, [$command->relatedTaskId]),
		);
	}
}
