...
class OnlyOfficeCustomServer extends AbstractCustomServer
{
			...
			if ($fetchResult->isSuccess())
			{
				...
				if (!is_string($this->version))
				{
					return $result->addError(new Error(
						message: Loc::getMessage('DISK_CUSTOM_SERVER_ONLYOFFICE_NO_SERVER_VERSION'),
					));
				}
			}
			else
	...
	public function prepareDataForConnect(): ?Error
	{
		...
		{
			return new Error(
				message: Loc::getMessage('DISK_CUSTOM_SERVER_ONLYOFFICE_INVALID_URL'),
			);
		}
		...
		if ($fetchVersionWithoutSecretKeyResult->isSuccess())
		{
			return new Error(
				message: Loc::getMessage(
					code: 'DISK_CUSTOM_SERVER_ONLYOFFICE_DISABLED_JWT',
					replace: [
						'#LINK#' => 'https://helpcenter.onlyoffice.com/docs/installation/docs-configure-jwt.aspx',
					],
				),
			);
		}
	...
	protected function fetchVersion(bool $useSecretKey = true): Result
	{
		...
		{
			return $result->addError(new Error(
				message: Loc::getMessage('DISK_CUSTOM_SERVER_ONLYOFFICE_NOT_CONFIGURED'),
				customData: [
		...
		{
			return $result->addError(new Error(
				message: Loc::getMessage('DISK_CUSTOM_SERVER_ONLYOFFICE_UNAVAILABLE'),
			));
		}
		...
		if ($httpClient->getStatus() !== 200)
		{
			return $result->addError(new Error(
				message: Loc::getMessage(
					code: 'DISK_CUSTOM_SERVER_ONLYOFFICE_UNAVAILABLE_WITH_STATUS',
					replace: [
		...
		if (isset($response['error']) && $response['error'] !== 0)
		{
			return $result->addError(new Error(
				message: Loc::getMessage(
					code: 'DISK_CUSTOM_SERVER_ONLYOFFICE_INVALID_JWT',
					replace: [
						'#LINK#' => 'https://helpcenter.onlyoffice.com/docs/installation/docs-configure-jwt.aspx',
					],
				),
