...
final class B24Boards extends Engine\Controller
{
	public function configureActions()
	{
		return [
			'verifyDomain' => [
				...
				'+postfilters' => [
					function () {
						(new Document\Flipchart\Configuration())->resetTempSecretForDomainVerification();
					}
				],
			],
		];
	...
	public function unregisterCloudClientAction(string $languageId): void
	{
		...
		$configuration = new Document\Flipchart\Configuration();
		...
		$cloudRegistration = (new Document\Flipchart\Cloud\Registration($serviceUrl))
		...
		if ($result->isSuccess())
		{
			...
			Option::set('disk', 'boards_enabled', 'N');
			Option::set('disk', 'boards_use_documentproxy', 'N');
			Option::set('disk', 'disk_boards_proxy_server', null);

			Document\Models\DocumentSessionTable::deleteByFilter([
				'=SERVICE' => Document\Models\DocumentService::FlipChart->value,
			]);
		}
		else
		{
	...
	public function registerCloudClientAction(string $serviceUrl, string $languageId): void
	{
		...
		if ($result->isSuccess() && isset($result->getData()['client']))
		{
			...
			Option::set('disk', 'boards_enabled', 'Y');
			Option::set('disk', 'boards_use_documentproxy', 'Y');
			Option::set('disk', 'disk_boards_proxy_server', $result->getData()['documentServer']['host']);
